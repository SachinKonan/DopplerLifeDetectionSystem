...
if __name__ == '__main__':
    ...
    lol = [ [x1[i], n[i]] for i in range(0, N)]
    radar = Radar(radar=lol, samplingRate=sRate)
    output = radar.main()
    mess = ''
    if(output['loc'] > 0.05 and output['loc'] < 3.3):
        ...
        mess = 'None'
    plt.plot(output['fft'][0], output['fft'][1])
    plt.plot(output['loc'],output['peak'], marker='x', color='r')
    plt.text(output['fft'][0][len(output['fft'][0]) - 1] - 3,output['peak'] - 1,  mess)
    plt.show()