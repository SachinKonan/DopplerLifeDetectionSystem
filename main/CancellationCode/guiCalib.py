...
class PageThree(tk.Frame):
    ...
    def SinwaveformGenerator(self):
      ...
      #add adc stuff here
      dac1.updateVal(cancelphase)
      dac2.updateVal(cancelgain)
    ...
    def RealtimePlotter(self):
      ...
      self.ax.axis([CurrentXAxis.min(),CurrentXAxis.max(),0,0.1])
      self.canvas.draw()
...
"""
    ...
    while gain >= -40:
        ...
        dac2.updateVal(vqreal)

        time.sleep(0.1)

        val = adc.getADCVAL(0)
        if(abs(val)  < abs(keygainvals[1])):
            keygainvals[0] = [vireal, vqreal,gain]
            keygainvals[1] = val
